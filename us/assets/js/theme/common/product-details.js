  ...
  constructor($scope, context, productAttributesData = {}) {
    ...
    $(".productView-thumbnails .productView-thumbnail").each(function () {
      if (!$(this).hasClass("playLoadedVideo")) {
        var imageobj = { src: $(this).find(".productView-thumbnail-link").attr("data-image-gallery-zoom-image-url") };
    ...
    $(".productView-img-container--custom.loadImageHere").on("click touchstart", function (e) {
      ...
      const { api } = Chocolat(lightboximages);
      api.open(index);
    });

    //Floating widget
    $("#addToCart-form-floating").on("submit", (e) => {
      e.preventDefault();
    ...
    const elementIsVisibleInViewport = (el, partiallyVisible = false) => {
      ...
      const newBottom = bottom - headerHeight;
    ...
    window.addEventListener("scroll", (event) => {
      let isvisible = elementIsVisibleInViewport(document.getElementById("form-action-addToCart"));
      if (isvisible) {
    ...
    utils.api.cart.itemAdd(normalizeFormData(new FormData(form)), (err, response) => {
      ...
      if (this.previewModal) {
        ...
        this.updateCartContent(this.previewModal, response.data.cart_item.id, () =>
          this.previewModal.setupFocusableElements(modalTypes.PRODUCT_DETAILS)
        );
      } else {
        this.$overlay.show();
        ...
        this.getCartContent( response.data.cart_item.id, (err, response) => {
          ...
          // Update cart counter
          let cartQuantity = document.querySelector(".custom-popup [data-cart-quantity]"),
